...
const UploadPage = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      // Ensure this endpoint matches your FastAPI router prefix
  ...
  return (
    /* h-screen + overflow-y-auto ensures the whole page scrolls if content overflows */
    <div className="h-screen w-full bg-white overflow-y-auto selection:bg-blue-100">
      <div className="max-w-2xl mx-auto p-8 md:p-16">
        ...
        <div className="relative group">
          ...
          />
          <div className="bg-zinc-50 border-2 border-zinc-200 rounded-3xl p-12 flex flex-col items-center justify-center transition-all group-hover:bg-zinc-100 group-hover:border-blue-400 border-dashed">
            ...
            <p className="text-zinc-400 text-sm mt-1">Maximum 10MB per file</p>
          </div>
        </div>
        ...
        {files.length > 0 && (
          <div className="mt-8 space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
            <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-400 px-1">
              Selected Documents ({files.length})
            </h3>
            
            {/* Scrollable list area for many files */}
            <div className="grid gap-2 max-h-72 overflow-y-auto pr-2 custom-scrollbar">
              {files.map((file, idx) => (
                <div key={idx} className="flex items-center justify-between bg-white border border-zinc-100 p-4 rounded-2xl shadow-sm hover:border-zinc-200 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-blue-50 text-blue-600 rounded-lg">
                      <FileText size={18} />
                    </div>
                    <div className="overflow-hidden">
                      <p className="text-sm font-medium text-zinc-800 truncate max-w-50 md:max-w-md">
                        {file.name}
                      </p>
